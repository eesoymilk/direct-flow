...
function generateOpinionSection(data: AuditReportData): AuditReportSection {
  ...
  const financialStatements = formatFinancialStatementPeriod(data.periodEnd, data.comparativePeriodEnd);
...
function getConcludingParagraph(data: AuditReportData): string {
  ...
  if (data.comparativePeriodEnd) {
    ...
    const compPerformancePeriod = formatIncomeStatementPeriod(data.comparativePeriodEnd);
...
export function generateAuditReportTemplate(data: AuditReportData): AuditReportTemplate {
  ...
  return {
    header: {
      ...
      period: formatFinancialStatementPeriod(data.periodEnd, data.comparativePeriodEnd),
      comparativePeriod: data.comparativePeriodStart && data.comparativePeriodEnd ? 
        formatBalanceSheetDate(data.comparativePeriodEnd) : undefined
    },
    sections,
    footer: {
