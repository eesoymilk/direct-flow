...
describe("Share Management System Integration", () => {
  ...
  describe("Initial State", () => {
    it("should initialize with only ordinary shares available", () => {
      const availableShareTypes =
        mockEnabledPreferredShares.length === 0
          ? ["ordinary"]
    ...
    it("should calculate zero total shares initially", () => {
      const totalShares = mockShareHoldings.reduce(
  ...
  describe("Sequential Preferred Share Enabling", () => {
    ...
    it("should enable preferred_b only after preferred_a", () => {
      ...
      expect(mockEnabledPreferredShares).toEqual([
        "preferred_a",
        "preferred_b",
      ]);
    });
  ...
  describe("Adding Preferred Shares", () => {
    ...
    it("should not allow adding disabled preferred shares", () => {
      ...
      const isPreferredBEnabled =
        mockEnabledPreferredShares.includes("preferred_b");
  ...
  describe("Multiple Shareholders with Mixed Holdings", () => {
    ...
    it("should calculate shares by type correctly", () => {
      const sharesByType = mockShareHoldings.reduce(
        (acc, holding) => {
    ...
    it("should calculate individual shareholder holdings", () => {
      const shareholder1Holdings = mockShareHoldings.filter(
        (h) => h.shareholderId === mockShareholders[0].id
      );
      const shareholder2Holdings = mockShareHoldings.filter(
        (h) => h.shareholderId === mockShareholders[1].id
      );

      expect(shareholder1Holdings).toHaveLength(2);
      expect(shareholder2Holdings).toHaveLength(1);

      const shareholder1Total = shareholder1Holdings.reduce(
        (sum, h) => sum + h.quantity,
        0
      );
      const shareholder2Total = shareholder2Holdings.reduce(
        (sum, h) => sum + h.quantity,
        0
      );
  ...
  describe("Share Removal and Disabling", () => {
    ...
    it("should remove holdings when disabling preferred share", () => {
      ...
      mockEnabledPreferredShares = mockEnabledPreferredShares.filter(
        (s) => s !== "preferred_b"
      );
    ...
    it("should maintain sequence when disabling last preferred share", () => {
      ...
      const lastEnabled =
        mockEnabledPreferredShares[mockEnabledPreferredShares.length - 1];
  ...
  describe("Validation Rules", () => {
    ...
    it("should validate share quantities are positive", () => {
    ...
    it("should validate share type availability", () => {
      ...
      const isAvailable =
        mockEnabledPreferredShares.includes(requestedShareType) ||
        requestedShareType === "ordinary";
  ...
  describe("Corporation System Integration", () => {
    ...
    it("should calculate ordinary vs preferred share distribution", () => {
      ...
      const ordinaryShares = mockShareHoldings
        .filter((h) => h.shareTypeId === 1)
        .reduce((sum, h) => sum + h.quantity, 0);

      const preferredShares = mockShareHoldings
        .filter((h) => h.shareTypeId > 1)
        .reduce((sum, h) => sum + h.quantity, 0);
