...
vi.mock("@nuxt/ui", () => ({
  UFormField: {
    name: "UFormField",
    template:
      '<div class="form-field"><label>{{ label }}</label><slot /></div>',
    props: ["label", "name", "required"],
  },
  URadioGroup: {
    name: "URadioGroup",
    template:
      '<div class="radio-group" @click="$emit(\'update:modelValue\', items[0]?.value)"><slot name="description" v-for="item in items" :item="item" /></div>',
  ...
  UCheckbox: {
    name: "UCheckbox",
    template:
      '<input type="checkbox" :checked="modelValue" @change="$emit(\'update:modelValue\', $event.target.checked)" class="checkbox" />{{ label }}',
  ...
  UInput: {
    name: "UInput",
    template:
      '<input :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" :type="type" :min="min" :placeholder="placeholder" :disabled="disabled" class="input" />',
  ...
  UInputNumber: {
    name: "UInputNumber",
    template:
      '<input :value="modelValue" @input="$emit(\'update:modelValue\', Number($event.target.value))" type="number" :min="min" :placeholder="placeholder" :disabled="disabled" class="input-number" />',
  ...
  UInputTags: {
    name: "UInputTags",
    template:
      '<input :value="modelValue?.join(\', \')" @input="$emit(\'update:modelValue\', $event.target.value.split(\', \'))" :placeholder="placeholder" :max="max" class="input-tags" />',
  ...
  UTextarea: {
    name: "UTextarea",
    template:
      '<textarea :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" :placeholder="placeholder" class="textarea"></textarea>',
...
vi.mock("../../app/components/company/application/helpers", () => ({
  organizationTypeItems: [
    ...
    },
    {
      ...
      label: "有限公司",
      description: "Limited company description",
    },
    {
...
describe("Organization Type Form Part 1", () => {
  ...
  describe("Capital and Shares Section", () => {
    ...
    it("should bind capital and share values correctly", async () => {
      ...
      if (totalSharesInput)
        expect(Number(totalSharesInput.element.value)).toBe(100000);
  ...
  describe("Form Field Interactions", () => {
    ...
    it("should handle shareholder count input for closely held corporations", async () => {
      ...
      const shareholderCountInput = wrapper.find(
        'input[placeholder*="股東人數"]'
      );
    ...
    it("should handle business description textarea", async () => {
      ...
      expect(store.formState.businessItemsDescription).toBe(
        "軟體開發、資訊服務"
      );
    });
  ...
  describe("Required Field Indicators", () => {
    it("should show required indicator for shareholder count when closely held", async () => {
      ...
      const shareholderCountField = formFields.find(
        (field: any) => field.props().name === "closelyHeldShareholderCount"
      );
