...
describe("Share System Validation", () => {
  ...
  describe("Corporation System", () => {
    ...
    it("should calculate share distribution", () => {
      ...
      const ordinaryShares = holdings
        .filter((h) => h.shareTypeId === 1)
        .reduce((sum, h) => sum + h.quantity, 0);
      const preferredShares = holdings
        .filter((h) => h.shareTypeId > 1)
        .reduce((sum, h) => sum + h.quantity, 0);
  ...
  describe("Business Logic Validation", () => {
    it("should validate share arithmetic calculations", () => {
      const currentShares = 450000;
      const newShares = 40000;
      const totalShares = currentShares + newShares;

      expect(totalShares).toBe(490000);

      const tooManyShares = 60000;
      const largerTotal = currentShares + tooManyShares;
      expect(largerTotal).toBe(510000);
    });

    it("should validate currency formatting", () => {
      const amount = 1250075.5;
